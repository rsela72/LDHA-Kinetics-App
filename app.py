...
# --- PAGE CONFIG ---
if "config_epsilon" not in st.session_state: st.session_state.config_epsilon = 6220.0
if "config_path_length" not in st.session_state: st.session_state.config_path_length = 1.0
if "config_enzyme_conc" not in st.session_state: st.session_state.config_enzyme_conc = 0.0
if "config_num_chunks" not in st.session_state: st.session_state.config_num_chunks = 8
...
if files:
    ...
    if set(current_file_names) != set(st.session_state.uploaded_file_names):
        ...
        summary_data = []
        for r in st.session_state.all_runs:
            summary_data.append({
                "Include": True,
                "File": r['filename'],
                ...
                "AS (sec)": round(r['as_time'], 3),
                "AE (sec)": round(r['ae_time'], 3)
            })
    ...
    if all_runs_from_session:
        ...
        for r in all_runs_from_session:
            summary_data.append({
                ...
                "AS (sec)": round(r['as_time'], 3), # Add AS here for editing
                "AE (sec)": round(r['ae_time'], 3)  # Add AE here for editing
            })

  
          

        with st.form("kinetics_form"):
                      
            st.warning("⚠️ **Important:** Edit your values below, then you MUST click **Calculate V0** to apply changes and update the plots.")
            ...
            edited_df = st.data_editor(
                ...
                width="stretch",
                column_config={
                    "Pyruvate (mM)": st.column_config.NumberColumn("Pyruvate (mM)", format="%.3f"),
                    "Enzyme Type": st.column_config.TextColumn("Enzyme Type"),
                    "AS (sec)": st.column_config.NumberColumn("AS (sec)", format="%.3f"),
                    "AE (sec)": st.column_config.NumberColumn("AE (sec)", format="%.3f")
                }
            )


            calculate_clicked = st.form_submit_button("Calculate V0")

        # Now the calculation only runs (and the app only reruns) when the button is clicked
        ...
        if calculate_clicked:
            ...
            for idx, row in edited_df.iterrows():
                ...
                if original_filename in included_filenames:
                    # Find the matching raw data in all_runs
                    run_state = next(r for r in st.session_state.all_runs if r['filename'] == original_filename)
                    
                    recalculated_res = calculate_kinetics(
                        run_state['full_df'],
                        original_filename,
                        user_as_time=row['AS (sec)'],
                        user_ae_time=row['AE (sec)']
                    )
                    
                    # Update the run_state for the downstream plots
                    run_state.update({
                        "pyruvate": row['Pyruvate (mM)'],
                        "enzyme_type": row['Enzyme Type'],
                        "as_time": row['AS (sec)'],
                        "ae_time": row['AE (sec)'],
                    ...
                    st.session_state.v0_calculated_results.append(run_state)
        ...
        if st.session_state.v0_calculation_done and st.session_state.v0_calculated_results:
            ...
            with st.expander("📝 View Detailed Calculations"):
                st.latex(r"V_0 (\mu M/s) = \frac{|\Delta Abs/sec|}{\epsilon \cdot l} \cdot 10^6")
                calc_table = []
                for r in final_results:
                    calc_table.append({
                        "Substrate [S]": f"{r['pyruvate']} mM",
                        "Slope (Abs/sec)": round(r['slope_abs_sec'], 6),
                        "Step 1: (Slope / \u03b5)": f"{r['slope_abs_sec']/EPSILON:.2e} M/sec",
            ...
            for idx, r_to_plot in enumerate(st.session_state.all_runs): # Iterate through session_state.all_runs for plotting
                ...
                if r_to_plot['filename'] in included_filenames: # Use included_filenames from data_editor
                    with cols[idx % 2].expander(f"Run: {r_to_plot['pyruvate']} mM Pyruvate ({r_to_plot['filename']})", expanded=False):
                        ...
                        ax2.plot(t, r_to_plot['slope_abs_sec'] * t + r_to_plot['intercept'], color='blue', lw=1)
